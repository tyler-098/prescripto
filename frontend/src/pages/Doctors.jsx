...
const Doctors = () => {
  ...
  return (
    <div>
      <p className='text-gray-600 '>Browse throught the doctors sepcialist.</p>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {
            filterDoc.map((item, index) => (
              <div onClick={() => navigate(`/appointments/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                ...
                <p className='text-gray-800 text-lg font-medium pl-3'>{item.name}</p>
                <p className='text-gray-600 text-sm pl-3 pb-3'>{item.speciality}</p>
